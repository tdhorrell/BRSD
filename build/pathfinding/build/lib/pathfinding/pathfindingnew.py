...
class NavigationNode(Node):
    ...
    def process_images(self):
        ...
        if SIMPLE_MODE:
            ...
            # Check if there is an active command sequence
            if self.command_sequence:
                # Get the current command
                command, duration = self.command_sequence[self.current_command_index]
                # Check if the duration has passed
                if duration is None:
                    # Indefinite duration, keep sending this command
                    ...
                    if elapsed_time >= duration:
                        ...
                        if self.current_command_index < len(self.command_sequence):
                            ...
                            navigation_command = [0, 0]  # Stop or default action
                            self.current_command_index = 0
                    else:
                        navigation_command = command
                ...
                # No active command sequence, check for person detections
                person_detections = self.get_person_detections(detections)
                if not person_detections:
                    self.get_logger().warn("Warning: No person detected in the image.")
                    self.determine_navigation_to_person(None)
                    return

                person_box = person_detections[0]  # Assuming the first detected person
                self.determine_navigation_to_person(person_box)
                # The function will set up the command sequence
        else:
            # Existing grid-based logic
            # Ensure the disparity image is available
            ...
            if not ranked_objects:
                ...
                self.path_pub.publish(Int32MultiArray(data=(0, 1)))
                return
            ...
            # 4. Determine Navigation Command for 3x3 Grid
            if GRID_SIZE == (3, 3):
                navigation_command = self.determine_3x3_navigation(grid)
                path_msg = Int32MultiArray(data=navigation_command)
                self.path_pub.publish(path_msg)
                self.get_logger().info(f"Published navigation command: {navigation_command}")

            # 5. Perform A* Pathfinding for 5x5 Grid
            elif GRID_SIZE == (5, 5):
                start = (GRID_SIZE[0] - 1, GRID_SIZE[1] // 2)  # Start position at bottom middle
                goal = (0, GRID_SIZE[1] // 2)  # Goal position at top middle
                path = self.astar(start, goal, grid)

                # Transform path for publishing if path exists
                if path:
                    transformed_path = [
                        (point[1] - 2, abs(point[0] - (GRID_SIZE[0] - 1)))
                        for point in path
                    ]
    ...
    def determine_navigation_to_person(self, person_box):
        if person_box is None:
            self.command_sequence = [([0,1], None)]
            self.command_start_time = time.time()
            self.current_command_index = 0
            self.get_logger().info("No person detected. Defaulting straight.")
            return
        ...
        # Determine which side the person is on
        if center_x < image_width / 3:
            # Person is on the left side
            # Set up sequence: turn right, sleep 0.5s, turn left, sleep 0.5s, go straight
            self.command_sequence = [
                ([1, 1], 0.5),  # Turn right for 0.5s
                ([-1, 1], 0.5),   # Turn left for 0.5s
                ([0, 1], None)   # Go straight indefinitely
            ]
            self.command_start_time = time.time()
            self.current_command_index = 0
            ...
            # Set up sequence: turn left, sleep 0.5s, turn right, sleep 0.5s, go straight
            self.command_sequence = [
                ([-1, 1], 0.5),   # Turn left for 0.5s
                ([1, 1], 0.5),  # Turn right for 0.5s
                ([0, 1], None)   # Go straight indefinitely
            ...
            # Default to turning right indefinitely
            self.command_sequence = [
                ([1, 1], 0.5),  # Turn right indefinitely
                ([-1,1], 0.5),
                ([0,1], None)
            ]
            self.command_start_time = time.time()
            self.current_command_index = 0
            self.get_logger().info("Person detected in the center. Defaulting to turning right.")
