...
class NavigationNode(Node):
    def __init__(self):
        ...
        self.disparity_sub_r = self.create_subscription(
            Image, '/multisense/right/disparity', self.disparity_image_callback_r, qos_profile)
    ...
    def disparity_image_callback_r(self, msg):
        # Convert the disparity image from ROS to OpenCV format
        self.disparity_image_r = self.bridge.imgmsg_to_cv2(msg, msg.encoding)
        self.disparity_image_r = cv2.resize(self.disparity_image_r, (640, 360))

        # Apply a binomial filter to reduce noise
        kernel_size = 5  # You can adjust the kernel size
        self.disparity_image_r = cv2.GaussianBlur(self.disparity_image_r, (kernel_size, kernel_size), 0)
        # try:
        #     filtered_disparity_msg = self.bridge.cv2_to_imgmsg(self.disparity_image_r, encoding='passthrough')
        #     filtered_disparity_msg.header = msg.header  # Preserve the original header
        #     #self.filtered_disparity_pub.publish(filtered_disparity_msg)
        #     self.get_logger().info("Published filtered disparity map.")
        # except CvBridgeError as e:
    ...
    def process_images(self):
        ...
        ranked_objects = self.rank_objects_by_distance(self.disparity_image, self.disparity_image_r, detections)
    ...
    def rank_objects_by_distance(self, disparity_image, disparity_image_r, detections):
        distance_data = []
        class_names = model.names  # {class_id: class_name}
        for detection in detections:
            ...
            roi2 = disparity_image_r[y1:y2, x1:x2]
            if roi.size > 0 and roi2.size >0:
                # Remove invalid disparities
                valid_disparities = roi[roi > 0]
                valid_disparities_r = roi[roi>0]
                if valid_disparities.size == 0 or valid_disparities_r.size == 0:
                ...
                median_disparity_r = np.median(valid_disparities_r)

                # Log the median disparity and associated object
                self.get_logger().info(
                    f"Object: {cls_name}, Bounding Box: [{x1}, {y1}, {x2}, {y2}], Median Disparity Left: {median_disparity}, Median Disparity Right: {median_disparity_r}"
                )
    ...
    def populate_astar_grid(self, disparity_image, ranked_objects):
        ...
        overlap_threshold = 0.15  # 15% threshold
