...
class NavigationNode(Node):
    ...
    def process_images(self):
        ...
        ranked_objects = self.rank_objects_by_distance(detections)
        ranking_time = time.time() - start_time
        self.get_logger().info(f"Object ranking time: {ranking_time:.4f} seconds")

        # 3. Populate Grid
        start_time = time.time()
        grid = self.populate_astar_grid(ranked_objects)
    ...
    def rank_objects_by_distance(self, detections):
        ranked_objects = []
        for detection in detections:
            ...
            # Log the bounding box area
            self.get_logger().info(
                f"Object detected with bounding box area: {bounding_box_area}, coordinates: ({x1}, {y1}, {x2}, {y2})"
            )

            # Store the bounding box area and coordinates
            ranked_objects.append((bounding_box_area, (x1, y1, x2, y2)))

        # Sort objects by bounding box area in descending order (larger boxes first)
        ranked_objects.sort(key=lambda x: -x[0])  # Negative for descending order
    ...
    def quantize_disparity_to_grid_row(self, disparity_value):
        ...
        if disparity_value < 700:
            grid_row = 6
        elif 700 <= disparity_value <= 1500:
            grid_row = 5
        else:  # disparity_value > 2000
            grid_row = 4
    ...
    def populate_astar_grid(self, ranked_objects):
        grid = np.zeros(GRID_SIZE, dtype=int)
        for _, (x1, y1, x2, y2) in ranked_objects:  # Ignore bounding box area
            # Map bounding box center to grid column
            col = int(((x1 + x2) / 2) / self.color_frame.shape[1] * GRID_SIZE[1])
            col = min(max(col, 0), GRID_SIZE[1] - 1)

            # Assign objects to a specific grid row (e.g., closest rows 6 or 5)
            grid_row = GRID_SIZE[0] - 1  # Default to row 6 (closest)
            ...
            # Logging
            self.get_logger().info(
                f"Placing object into grid cell ({grid_row}, {col})"
            )
        return grid
