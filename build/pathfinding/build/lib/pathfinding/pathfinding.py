...
class NavigationNode(Node):
    ...
    def rank_objects_by_distance(self, disparity_image, detections):
        ...
        for detection in detections:
            ...
            if roi.size > 0:
                ...
                median_disparity = np.mean(valid_disparities)
    ...
    def populate_astar_grid(self, disparity_image, ranked_objects):
        ...
        for disparity_value, (x1, y1, x2, y2) in ranked_objects:
            ...
            # Map x position to grid column
            col = int(((x1 + x2) / 2) / disparity_image.shape[1] * GRID_SIZE[1])
            col = min(max(col, 0), GRID_SIZE[1] - 1)
            grid[grid_row][col] = 1  # Mark cell as occupied

            # Add logging here
            self.get_logger().info(
                f"Placing object with disparity {disparity_value} into grid cell ({grid_row}, {col})"
            )
        return grid
